...
class PluginCardsTagTestCase(TestCase):
    ...
    def test_bad_card_shows_debug_placeholder_in_debug_mode(self):
        plugin = _make_plugin_config(
            name="bad_plugin", label="bad_plugin", babybuddy_has_dashboard_card=True
        )
        with mock.patch(
            ...
            result = self._call_tag([plugin])
        self.assertIn("Plugin card error", result)
        self.assertIn("bad_plugin", result)

    @override_settings(DEBUG=False)
    def test_bad_card_renders_nothing_in_production(self):
        plugin = _make_plugin_config(
            name="bad_plugin", label="bad_plugin", babybuddy_has_dashboard_card=True
        )
        with mock.patch(
